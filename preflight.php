...
|--------------------------------------------------------------------------
|
| This is Preflight's own config — it dogfoods every built-in step, plus the
| opt-in Deptrac, which isn't in the default set:
|
|   Deptrac  enforces the layer boundaries  (see deptrac.yaml)
|
| addSteps() keeps the eight auto-detected defaults and appends Deptrac, so we
| don't restate the whole pipeline. tune() reconfigures the Tests step to also
| emit coverage reports (CI uploads them as an artifact). Each step still runs
| only if its tool is installed; coverage needs a driver, and without one the
| tests still run with a non-failing warning.
*/
